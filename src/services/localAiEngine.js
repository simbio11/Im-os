...
/**
 * High-Intelligence Built-in Answer Generator
 * Analyzes intent, keywords, and live OS state to produce concise, crisp, and direct answers.
 */
export function queryLocalAiEngine(query, {
  ...
  // 1. AI Identity & Capabilities ("넌 뭘 할 수 있어?", "자기소개", "도움말", "기능")
  if (q.includes('뭘 할 수') || q.includes('누구') || q.includes('도움말') || q.includes('기능') || q.includes('소개') || q.includes('help') || q.includes('what can you do')) {
    return {
      answer: `안녕하세요! **L&M OS 최고 전략 AI 비서**입니다. 사용자의 모든 라이프 프로토콜과 시장 데이터를 실시간으로 파악하고 있습니다.
...
궁금하신 점을 편하게 말씀해 주세요!`,
      sources: ["L&M OS Intelligence Kernel", "Executive Capabilities"]
    };
  }

  // 2. Schedule Q&A ("학회는 언제 가지", "내일 일정", "오늘 일정", "몇 시", "언제")
  // Check if query is asking about a specific event or day
  ...
  if (isScheduleQuery) {
    // A. Specific event keyword search (e.g. "학회는 언제 가지", "판교 언제 가", "기상 몇 시")
    ...
    if (matchedKeyword) {
      ...
      if (matchedEvents.length > 0) {
        ...
        const dateLabel = first.date === todayStr ? '오늘' : first.date === tomorrowStr ? '내일' : `${first.date}`;
  ...
  // 3. Finance & Investment Surplus ("잉여금", "가계부", "투자", "소득", "고정비", "배분")
...
3. **유동성 및 기회 자금 (CMA/단기채 15%)**: **${cmaAmt.toLocaleString()}원**`,
  ...
  // 4. Diet, Nutrition & Running ("식단", "칼로리", "단백질", "러닝", "5km", "운동")
...
💡 **피드백**: 저녁 식사에서 양질의 단백질(닭가슴살/생선) 30g을 추가 보충하시면 일일 권장량을 완벽히 달성합니다.`,
  ...
  // 5. Stock & Macro Intelligence ("금리", "국채", "엔비디아", "nvda", "블랙웰", "나스닥", "기술주", "fomc")
...
4. **전략**: 연준의 9월 금리 인하 사이클 진입에 맞춰 QQQ/NVDA 적립식 매수 유지.`,
  ...
  // 6. Routine & Deepwork ("루틴", "딥워크", "포모도로", "레벨", "xp", "스트릭")
...
• **딥워크 추천**: 40Hz 감마파 바이노럴 비트와 함께 90분 집중 블록을 수행하면 +100 XP가 지급됩니다.`,
  ...
  // 7. PubMed & Medical Search ("논문", "pubmed", "연구", "의학", "세포", "미토콘드리아")
  if (q.includes('논문') || q.includes('pubmed') || q.includes('의학') || q.includes('연구') || q.includes('미토콘드리아') || q.includes('노화')) {
    const papers = PUBMED_PAPERS_DB.slice(0, 2);
    const paperList = papers.map(p => `• **[${p.pmid}] ${p.title}** (${p.journal})\n  - *핵심 결론*: ${p.abstract.slice(0, 100)}...`).join('\n\n');
    return {
      answer: `🔬 **PubMed 학술 논문 데이터베이스 검색 결과:**\n\n${paperList}`,
      sources: ["PubMed Knowledge Index", "NCBI BioC Engine"]
    };
  }
